...
def main():
    ...
    general_set_sizes = [80]
    num_repair_inputs_list = [1, 2, 4, 8, 16, 32]
    gammas = [1]

    this_calc_margins = partialmethod(_calc_margins, target_margin=0.0)
    Repairer._calc_margins = this_calc_margins

    for gamma in gammas:
        results_dir = os.path.join(os.path.dirname(__file__), f"../benchmark_results/resnet_large_repair_edge_repeated_{gamma}/")
            ...
            for num_repair_inputs in num_repair_inputs_list:
                ...
                with open(os.path.join(results_dir, results_file), "w") as _:
                    ...
                    for repair_class in range(10):
                        ...
                        for i in range(specific_labels.shape[0]):
                            repairer = Repairer(model, general_inputs, general_labels, specific_inputs[i:i+1],
                                                specific_labels[i:i+1], lr=1e-5)
