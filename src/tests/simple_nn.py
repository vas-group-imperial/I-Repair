...
class SimpleNN(FFNN):
    ...
    def __init__(self, activation="Sigmoid"):
        ...
        """
        Args:
            activation: The activation function, choose from ["Sigmoid", "Tanh",
            "Relu"]
        """

        if activation == "Sigmoid":
            hidden_1 = nn.Sequential(nn.Linear(2, 2), nn.Sigmoid())
            hidden_2 = nn.Sequential(nn.Linear(2, 1))
...
# noinspection PyUnresolvedReferences
class SimpleNN2(FFNN):

    """
    A simple torch fully connected neural network with 1 input, 2 hidden and 2
    and 2 output nodes.
    """
...
# noinspection PyUnresolvedReferences
class SimpleNNBatchNorm2D(FFNN):
...
class SimpleReLU2Lay(FFNN):
    ...
    def __init__(self, activation="Sigmoid"):
        ...
        hidden_1 = nn.Sequential(nn.Linear(2, 2), nn.ReLU())
        hidden_2 = nn.Sequential(nn.Linear(2, 2), nn.ReLU())
        out = nn.Sequential(nn.Linear(2, 2))

        # noinspection PyArgumentList
        list(hidden_1.children())[0].weight.data = torch.Tensor([[1, -1], [1, 1]])
        # noinspection PyArgumentList
        list(hidden_2.children())[0].weight.data = torch.Tensor([[1, 1], [-1, 1]])
        # noinspection PyArgumentList
        list(out.children())[0].weight.data = torch.Tensor([[1, 1], [-1, -1]])

        # noinspection PyArgumentList
        list(hidden_1.children())[0].bias.data = torch.Tensor([1, -1])
        # noinspection PyArgumentList
        list(hidden_2.children())[0].bias.data = torch.Tensor([0.5, -0.5])
        # noinspection PyArgumentList
        list(out.children())[0].bias.data = torch.Tensor([0.5, 0.5])

        super(SimpleReLU2Lay, self).__init__([hidden_1, hidden_2, out])
