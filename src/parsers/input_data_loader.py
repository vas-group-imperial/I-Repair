...
def load_pckl_mnist(path: str, img_nums: list) -> np.array:

    images = np.zeros((len(img_nums), *(28, 28)), dtype=float)
    labels = np.zeros((len(img_nums)), dtype=int)

    for idx, i in enumerate(img_nums):
        with open(path + f"im{i}.pkl", 'rb') as pickle_file:
            data = pickle.load(pickle_file)

        images[idx, :, :] = data[0].reshape((28, 28))
        labels[idx] = data[1]
