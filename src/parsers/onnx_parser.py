...
OBS:
This code only supports standard architectures where the network has one input,
several hidden and one output layer. The input to each layer is expected to be the
output from the previous layer. FC and Conv2d layers as well as ReLU, Sigmoid and
Tanh activation functions are supported.

Flattening/ reshaping are skipped and handled in the PyTorch _model. The PyTorch _model
assumes that the network is either (i) fully connected and does not require reshaping,
(ii) fully convolutional and does not require reshaping or (iii) Convolutional followed
by fully connected and flattening is performed in the transition convolutional to fc.

Other architectures/activations/computational graphs are not handled and may lead to
...
class ONNXParser:
    ...
    def to_pytorch(self) -> FFNN:

        """
        Converts the self.onnx _model to a FFNN(torch) _model.
        ...
        self.torch_model = FFNN(mappings)
    ...
    def _process_node(self, curr_input_idx: int, node: onnx.NodeProto) -> torch.nn:
        ...
        if node.op_type == "Relu":
            ...
            # Reshape operations are assumed to adhere to the standard used in FFNN and thus skipped.
            curr_input_idx = node.output[0]
    ...
    def conv_to_torch(self, node) -> nn.Linear:
        ...
        """
        Converts a onnx 'Conv' node to a torch Conv node.
