...
class ResNet(nn.Module):
    ...
    def __init__(self, num_blocks: tuple, use_gpu: bool = False):
        ...
        self.layer1 = self._make_layer(BasicBlock, 32, 32, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(BasicBlock, 32, 64, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(BasicBlock, 64, 128, num_blocks[2], stride=2)
    ...
    def _init_dataloaders(self,  num_train: int = 90000,
                          data_dir: str = "../../resources/tiny-imagenet/tiny-imagenet-200"):
        ...
        val_trans = transforms.Compose([normalize])

        self.dset_train = TinyImageNetDSet(root=data_dir, train=True, transform=train_trans)
        self._train_loader = DataLoader(self.dset_train, batch_size=64,
                                        sampler=sampler.SubsetRandomSampler(range(num_train)))

        self.dset_val = TinyImageNetDSet(root=data_dir, train=True, transform=val_trans)
        self._val_loader = DataLoader(self.dset_val, batch_size=64,
                                      sampler=sampler.SubsetRandomSampler(range(num_train, 100000)))

        self.dset_test = TinyImageNetDSet(root=data_dir, train=False, transform=val_trans)
        self._test_loader = DataLoader(self.dset_test, batch_size=100)
    ...
    def save(self, path: str = "../../resources/models/tiny_imagenet_resnet_large.pth"):
    ...
    def load(self, path: str = "../../resources/models/resnet_large.pth"):
